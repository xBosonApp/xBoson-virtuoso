...
function createCore() {
  ...
  function run(rc, RCB, _identify, __is_test, _log_ext) {
    ...
    function _onStop(handle) {
      main_event_link.once(ev.STOP_HANDLE, function() {
        try {
          heanle();
        } catch(err) {
    ...
    function _invokeTarget(next_interactive) {
      ...
      try {
        ...
        runstate.change(STATE.ERROR);
        hisdat.msg = "目标抛出异常:" + error.message;
        hisdat.data = error;
        hisdat.time = Date.now();
        _history(hisdat);
      }
    }
    ...
    function _begin(_being_target) {
      ...
      main_event_link.once(ev.L_EX_ROWCOUNT, function(_dat) {
        _dat.time = new Date();
        log.cnt(_dat);
      });
...
function createInteractive(runtime_context, _flowData, parent_event_link, _sub_id) {
  ...
  // code   - 错误代码
  //
  function sendError(errObj, data, code) {
